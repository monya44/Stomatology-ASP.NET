...
public class UserRolesController : Controller
{
    ...
    public async Task<IActionResult> Update(string id, ManageUserRolesViewModel model)
    {
        ...
        return RedirectToAction("Index", "Users");// new { userId = id }
        }
    public async Task<IActionResult> Delete(string id, ManageUserRolesViewModel model)
    {
        var user = await _userManager.FindByIdAsync(id);
        var roles = await _userManager.GetRolesAsync(user);
        var result = await _userManager.RemoveFromRolesAsync(user, model.UserRoles.Where(x => x.Selected).Select(y => y.RoleName));
        var currentUser = await _userManager.GetUserAsync(User);
        await _signInManager.RefreshSignInAsync(currentUser);
